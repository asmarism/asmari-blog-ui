...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen text-right myriad-font" dir="rtl">
        ...
        <div className="max-w-md mx-auto px-6 relative flex justify-between items-center min-h-[44px]">
          ...
          <div className={`flex items-center gap-3 transition-all duration-300 transform-gpu ${isSearchOpen ? 'opacity-0 scale-95 pointer-events-none' : 'opacity-100 scale-100'}`}>
            {selectedPost && (
              <button 
                onClick={handleBack}
                className="p-2 liquid-glass rounded-xl text-[#94A3B8] active:scale-90 transition-all"
              >
                <ArrowRight size={18} />
              </button>
            )}
            <button onClick={handleTitleClick} className="flex items-baseline gap-2 text-right group">
              ...
              <p className={`myriad-font font-normal transition-all duration-500 text-slate-400 whitespace-nowrap ${
                isScrolled || selectedPost ? 'text-[10px]' : 'text-[12px]'
              }`}>
                بقلم سلمان الأسمري
              </p>
            </button>
          ...
          >
            <div className="relative w-full h-full">
              <input
                ...
                className={`w-full h-full bg-white/5 border rounded-[20px] transition-all duration-500 pr-11 pl-4 text-sm text-white focus:outline-none myriad-font ${
                  isSearchOpen 
              ...
              <button 
                ...
                className={`absolute right-0 top-0 w-[42px] h-[42px] flex items-center justify-center transition-all duration-500 rounded-xl ${
                  isSearchOpen ? 'text-slate-400' : 'liquid-glass text-slate-400 hover:text-white'
                }`}
            ...
            <button 
              ...
              className={`whitespace-nowrap text-xs font-bold text-[#FFA042] px-1 transition-all duration-300 myriad-font ${
          ...
          >
            <article className="pt-16">
              <div className="mb-6 text-right">
                <span className="text-[10px] font-bold text-[#94A3B8] block mb-1 opacity-80 uppercase tracking-widest myriad-font">
                  {selectedPost.date}
                </span>
              ...
              <div 
                className="wp-content text-slate-200 text-[16px] leading-[1.8] space-y-4 text-right myriad-font"
                ...
                <section className="mt-16 pt-10 border-t border-white/10 text-right">
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {recommendedPosts.map((rp) => (
                      <div 
                        key={rp.id}
                        onClick={() => handlePostClick(rp)}
                        className="liquid-glass p-4 rounded-2xl flex flex-row-reverse gap-4 items-center cursor-pointer hover:border-[#1B19A8]/30 transition-all active:scale-[0.98] text-right"
                      >
            ...
            <div className="mt-16 flex flex-col items-center gap-4 opacity-30 pb-40 text-center animate-bounce">
              ...
              <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest myriad-font">اسحب للعودة للرئيسية</span>
            </div>
          </div>
        )}
        ...
        <div className={`transition-all duration-700 ${selectedPost ? 'hidden pointer-events-none' : 'block animate-in fade-in slide-in-from-bottom-4'}`}>
          <section className={`transition-all duration-1000 ease-[cubic-bezier(0.23,1,0.32,1)] overflow-hidden ${
            ...
            <div className="space-y-1.5 text-right">
              ...
              <p className="text-[13px] leading-[1.7] text-slate-300/90 font-normal myriad-font">
          ...
          <section className="mb-5 sticky top-[54px] z-40 pb-2.5 pt-0">
            <div className="grid grid-cols-3 gap-2">
              {categories.map((cat) => (
                <button
                  key={cat.name}
                  onClick={() => { setActiveCategory(cat.name); setIsSearchOpen(false); }}
                  className={`flex items-center justify-center gap-1 px-2 py-2.5 rounded-xl transition-all duration-300 border ${
                ...
                >
                  <article>
                    ...
                    <div className="p-5 text-right">
                      ...
                      <p className="text-xs text-slate-400 line-clamp-2 leading-relaxed opacity-90 mb-4 myriad-font">
                        {post.excerpt}
                      </p>
      ...
      {!selectedPost && (
        <footer className="max-w-md mx-auto px-6 pb-10 pt-6 relative z-10">
          <FooterContent />
        </footer>
      )}
